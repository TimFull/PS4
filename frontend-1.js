...
function makeUI()
{
    ...
    italic.appendChild(document.createTextNode('点击左侧的用户名可生成随机ID，或手动编辑。点击右侧OK按钮生效。'));
...
function fetchAccount(i)
{
    ...
    if(!mailbox.account_name)
    {
        ...
        italic.appendChild(document.createTextNode('帐户不存在'));
        ui[i].id.value = '';
